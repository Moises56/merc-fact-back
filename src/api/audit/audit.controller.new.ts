...
export class AuditController {
  ...
  @Roles(Role.ADMIN, Role.AUDITOR)
  @ApiOperation({ summary: 'Obtener registros de auditoría por tipo de entidad' })
  @ApiResponse({
    status: 200,
    description: 'Registros de auditoría del tipo de entidad',
  })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  async findByEntityType(
    ...
    try {
      ...
      return await this.auditService.findByEntityType(entityType, pageNum, limitNum);
    } catch (error) {
      console.error('Error fetching entity audit logs:', error);
      throw new HttpException(
        'Error al obtener registros de auditoría por entidad',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
